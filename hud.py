...
class TacticalHUD:
    def __init__(self, root, config):
        ...
        self.canvas = tk.Canvas(self.win, width=460, height=195, bg="#ff00ff", highlightthickness=0)
        self.canvas.pack()
    ...
    def update(self, current_sys, dest_name, dist_ly, scanned, total, edsm_status, r_pos, organic_count, system_traffic, game_r_pos=None):
        ...
        self.draw_text(20, 80, text=dest_txt, fill=COLOR_ORANGE, font=("Courier", 10, "bold"), anchor="w")
        self.draw_text(440, 80, text=dist_ly, fill=COLOR_ORANGE, font=("Courier", 10, "bold"), anchor="e")
        ...
        self.draw_text(20, 135, text=f"SCAN: {scanned}/{total} BODIES", fill=COLOR_TEXT, font=("Courier", 9), anchor="w")
        self.draw_text(440, 135, text=f"{int(pct*100)}%", fill=COLOR_ACCENT, font=("Courier", 10, "bold"), anchor="e")
        
        t_day = system_traffic.get('day', 0)
        t_week = system_traffic.get('week', 0)
        t_total = system_traffic.get('total', 0)
        self.draw_text(20, 155, text=f"TRAFFIC: Today : {t_day}  This Week : {t_week}  Total : {t_total}", fill=COLOR_TEXT, font=("Courier", 9), anchor="w")

        status_color = COLOR_GREEN if edsm_status == "OK" else "red"
        self.draw_text(20, 175, text=f"EDSM: {edsm_status}", fill=status_color, font=("Courier", 9, "bold"), anchor="w")
        ...
        self.draw_text(440, 175, text=route_text, fill=COLOR_GREEN, font=("Courier", 9, "bold"), anchor="e")