...
*   Refined the Navigation HUD centre-state display into a single theme-aware data row: balanced route and known-system-survey tracers now combine with distinct restrained signatures for flight, supercruise, jumps, docking, landing, on-foot travel, SRV/Nomad driving, scanners, maps and combat in both normal and compact layouts, with reduced-motion and hidden-overlay safeguards; retired the unrelated far-right title pip so the new instrument can use the full remaining header width.
