...
class ExpeditionMapView:
    ...
    def __init__(self, parent, app, open_record_callback=None):
        ...
        self._annotations = []
        self._view_state = self._normalise_view_state(self.config.get('explore_map_view_state'))
    ...
    def _normalise_annotations(rows):
        ...
        for index, row in enumerate(rows or ()):
            ...
            if category not in ANNOTATION_TYPES:
                category = 'Note'
            output.append({'id': _bounded_text(row.get('id') or f'legacy-{index}', 100), 'category': category, 'title': _bounded_text(row.get('title') or category, 120), 'note': _bounded_text(row.get('note'), 2000), 'system': _bounded_text(row.get('system'), 120), 'position': list(position), 'created': _bounded_text(row.get('created'), 80)})
        return output
    ...
    def _light_route_row(row):
        ...
        if position is None:
            return None
        return {'system': _bounded_text(row.get('system'), 120), 'pos': list(position), 'timestamp': _bounded_text(row.get('timestamp'), 80), 'jump_dist': round(float(row.get('jump_dist') or 0.0), 3), 'star_class': _bounded_text(row.get('star_class'), 40), 'fss_complete': bool(row.get('fss_complete'))}
    ...
    def _build_markers(self, survey_snapshot, bookmarks, positions):
        ...
        def add(layer, kind, system='', subject='', detail='', position=None, **extra):
            ...
            if position_value is None:
                return
            row = {'layer': layer, 'kind': kind, 'system': _bounded_text(system, 120), 'subject': _bounded_text(subject or kind, 160), 'detail': _bounded_text(detail, 500), 'position': list(position_value)}
            row.update(extra)
            markers.append(row)
        ...
        for system, rows in grouped_values.items():
            ...
            add('Valuable', 'Valuable', system, top.get('body') or f'{len(rows)} valuable worlds', f"{len(rows)} retained · top {int(top.get('value') or 0):,} cr")
        for row in self._system_rows:
            signals = int(row.get('bio_signals') or 0)
            if signals:
                add('Biology', 'System', row.get('system'), 'Biological survey', f"{signals} biological signal{('s' if signals != 1 else '')}")
        ...
        for system, rows in grouped(survey_snapshot.get('codex')).items():
            add('Codex', 'Codex', system, rows[-1].get('name') or 'Codex records', f'{len(rows)} Codex record(s)')
        for system, rows in grouped(survey_snapshot.get('screenshots')).items():
            add('Photos', 'Photo', system, rows[-1].get('body') or 'Screenshot', f'{len(rows)} screenshot(s)')
        for row in survey_snapshot.get('candidates') or ():
            add('Recon', 'Recon', row.get('system'), 'Recon candidate', f"{int(row.get('score') or 0)}/100 {row.get('grade') or ''}")
        for row in survey_snapshot.get('revisit_queue') or ():
            add('Revisit', 'Revisit', row.get('system'), 'Unfinished exploration', row.get('detail') or 'Worthwhile survey work remains', position=row.get('position'))
        for row in bookmarks:
            add('Bookmarks', 'Bookmark', row.get('system'), row.get('title') or row.get('kind') or 'Bookmark', ' · '.join(filter(None, (str(row.get('priority') or ''), ', '.join(row.get('tags') or [])))), position=row.get('position'), bookmark_id=row.get('id'))
        for row in self._annotations:
            add('Annotations', 'Annotation', row.get('system'), row.get('title'), row.get('note') or row.get('category'), position=row.get('position'), annotation_id=row.get('id'), category=row.get('category'), note=row.get('note'))
        ...
        if isinstance(plan, dict) and plan.get('center'):
            grid = sector_grid(survey_snapshot.get('route_points') or (), plan.get('center'), plan.get('radius_ly', 500), plan.get('cell_size_ly', 100))
            ...
            for row in important + untouched[:allowance]:
                ...
                add('Sectors', 'Sector', '', f"{plan.get('name') or 'Expedition sector'} · cell {row.get('id')}", f"{status.title()} · {int(row.get('surveyed_systems') or 0)}/{int(row.get('visited_systems') or 0)} visited systems FSS complete", position=row.get('position'), status=status, cell_size=float(grid.get('cell_size_ly') or plan.get('cell_size_ly') or 100))
        return markers
    ...
    def _build_snapshot(self):
        ...
        self._annotations = self._normalise_annotations(self.config.get('explore_map_annotations'))
        ...
        for row in planned_context.get('planned') or ():
            ...
            if position is None:
                continue
            planned.append({'system': _bounded_text(row.get('system'), 120), 'pos': list(position), 'source': _bounded_text(row.get('source'), 30), 'visited': bool(row.get('visited'))})
            if row.get('system'):
                positions[str(row['system']).casefold()] = position
    ...
    def refresh(self, system_rows=None, value_rows=None):
        if self._disposed:
            return
        if system_rows is None and hasattr(self.app, 'db_lock'):
            if self.app.db_lock.acquire(blocking=False):
                try:
                    counts = defaultdict(int)
                    for system, payload in self.app.conn.execute('SELECT system_name, data_json FROM scan_hud_items'):
                        item = json.loads(payload)
                        counts[system] += int(item.get('bio_count') or 0)
                    system_rows = [{'system': system, 'bio_signals': count} for system, count in counts.items()]
                finally:
                    self.app.db_lock.release()
        if value_rows is None and hasattr(self.app, '_html_ledger_workspace'):
            value_rows = self.app._html_ledger_workspace().get('rows', [])
        ...
        try:
            ...
            logging.exception('Galactic Atlas snapshot failed')
            return
        ...
        if region.get('name'):
            current_text += f" // REGION {int(region.get('id') or 0):02d} {region['name'].upper()}"
        pass
    ...
    def focus_system(self, system):
        ...
        row = next((row for row in snapshot.get('route') or () if str(row.get('system') or '').casefold() == wanted), None)
        if row is None:
            row = next((marker for marker in snapshot.get('markers') or () if str(marker.get('system') or '').casefold() == wanted), None)
        position = _position((row or {}).get('pos') or (row or {}).get('position'))
        if position is None:
            return False
        self._focus_sequence += 1
        self._focus_request = {'id': self._focus_sequence, 'system': _bounded_text(system, 120), 'position': list(position)}
        self.refresh()
        return True

    def _focus_current(self):
        current = self._latest_snapshot.get('current') or {} if self._latest_snapshot else {}
        position = _position(current.get('position'))
        if position is None:
            return
        self._focus_sequence += 1
        self._focus_request = {'id': self._focus_sequence, 'system': current.get('system') or 'Current position', 'position': list(position)}
        self.refresh()
    ...
    def has_live_browser(self, grace_seconds=20.0):
        ...
        if clients > 0:
            return True
        return bool(seen and time.monotonic() - float(seen) <= max(0.0, float(grace_seconds)))
    ...
    def _handle_browser_command(self, payload):
        ...
        if action == 'annotation_upsert':
            ...
            row = {'id': annotation_id, 'category': category, 'title': _bounded_text(raw.get('title') or category, 120), 'note': _bounded_text(raw.get('note'), 2000), 'system': _bounded_text(raw.get('system'), 120), 'position': list(position), 'created': _bounded_text(raw.get('created'), 80) or datetime.now().astimezone().isoformat()}
            self._annotations = [item for item in self._annotations if item.get('id') != annotation_id] + [row]
            self.config['explore_map_annotations'] = [dict(item) for item in self._annotations]
        ...
        if action == 'annotation_delete':
            ...
            existing = next((row for row in self._annotations if row.get('id') == annotation_id), None)
            if existing is None:
                return
            self._annotations = [row for row in self._annotations if row.get('id') != annotation_id]
            self.config['explore_map_annotations'] = [dict(item) for item in self._annotations]
            self._persist_annotations()
            feed = getattr(self.app, 'add_event_feed_entry', None)
            if callable(feed):
                feed('MAP', f"Annotation deleted: {existing.get('title') or 'Map mark'}", severity='INFO')
            self.refresh()
            return
        if action == 'open_record':
            record = payload.get('record')
            if isinstance(record, dict) and callable(self.open_record_callback):
                self.open_record_callback({key: record.get(key) for key in ('kind', 'system', 'subject', 'detail', 'bookmark_id', 'annotation_id', 'category', 'position')})
    ...
    def _schedule_status_tick(self):
        ...
        pass
        try:
            self._status_job = self.parent.call_later(250, self._schedule_status_tick)
        except RuntimeError:
            self._status_job = None
    ...
    def dispose(self):
        ...
        if self._status_job is not None:
            try:
                self.parent.cancel(self._status_job)
            except RuntimeError:
                pass
        self._status_job = None
        if hasattr(self, 'server'):
