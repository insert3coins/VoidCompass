"""Shared lifecycle bridge for purpose-built semantic HTML overlays."""
...
class HtmlModelOverlayBridge:
    ...
    def __init__(
        ...
        template, snapshot_key, model_attr, log_name, width,
        ...
        self.snapshot_key = str(snapshot_key)
    ...
    def _snapshot(self):
        ...
        payload = {
            ...
            "effects": {
                ...
                "reduced_motion": bool(
                    self.config.get("reduced_motion_enabled", False)
                ),
        ...
        payload[self.snapshot_key] = _json_safe(self._model())
        return payload

    def _quick_fingerprint(self):
        model = json.dumps(
            self._model(), sort_keys=True, ensure_ascii=False, default=str,
        )
        palette = tuple(sorted(
            (getattr(self.overlay, "_palette", {}) or {}).items()
        ))
        return (
    ...
    def _sync(self):
        ...
        if surface is not None:
            if surface.startup_failed:
                ...
                if self._ready:
                    if not was_ready:
                        logging.info("HTML %s renderer is live", self.log_name)
...
def attach_html_model_overlay(
    ...
    setattr(overlay, bridge_attr, bridge)
