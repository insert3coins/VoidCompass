...
from voidcompass.exploration.galactic_regions import find_region, region_names
...
def system_completion(
    ...
    return {
        ...
        "complete": complete,
        "percent": percent,
        "known_bodies": len(bodies),
        "mapped_bodies": sum(1 for row in rows if row["mapped"]),
        "bio_targets": sum(1 for row in rows if row["bio_total"]),
        "geo_targets": sum(1 for row in rows if row["geo_detected"]),
        "valuable_targets": sum(1 for row in bodies if _is_valuable(row)),
