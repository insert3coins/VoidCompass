...
class GalacticMapServer:
    ...
    def allow_frame_ancestor(self, origin):
        """Allow one exact loopback application origin to embed the atlas."""
        ...
        with self._condition:
            self._frame_ancestors.add(normalised)
        return True
    ...
    def _security_headers(self, handler):
        with self._condition:
            frame_ancestors = sorted(self._frame_ancestors)
        ancestor_policy = " ".join(frame_ancestors) if frame_ancestors else "'none'"
        ...
        handler.send_header(
            ...
            f"object-src 'none'; frame-ancestors {ancestor_policy}; base-uri 'none'",
        )
    ...
    def _handle_get(self, handler):
        ...
        if path == "/assets/atlas.png":
            ...
            candidate = (
                self.static_root.parent.parent / "assets" / "icons" / "icon-source.png"
            )
        else:
