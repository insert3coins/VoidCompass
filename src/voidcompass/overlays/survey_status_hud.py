...
from voidcompass.exploration.notable_bodies import build_notable_body_rows
_CHROMA = '#ff00ff'
WIDTH = 420
...
def _survey_bodies(scan_items, body_signals):
    ...
    for body in bodies:
        ...
        body['mining_count'] = _safe_int(signals.get('mining'), _safe_int(body.get('mining_count')))
        body['dss_complete'] = bool(body.get('dss_complete') or signals.get('dss_complete'))
    ...
    for body_id, signals in (body_signals or {}).items():
        ...
        if not bio_count and (not geo_count) and (not mining_count):
            continue
        bodies.append({'body_id': body_id, 'name': signals.get('body_name') or f'Body {body_id}', 'bio_count': bio_count, 'geo_count': geo_count, 'mining_count': mining_count, 'genuses': list(signals.get('genuses') or []), 'organic_scans': {}, 'organic_complete_count': 0, 'dss_complete': bool(signals.get('dss_complete')), 'dss_probes_used': signals.get('dss_probes_used'), 'dss_efficiency_target': signals.get('dss_efficiency_target'), 'dss_efficiency_met': signals.get('dss_efficiency_met')})
    return bodies

def _genus_name(value):
    if isinstance(value, dict):
        return value.get('Genus_Localised') or value.get('Name_Localised') or value.get('name') or value.get('Genus') or value.get('Name')
...
def _surface_signal_state(bio_count=0, complete=0, geo_count=0, mining_count=0, needs_dss=False, palette=None):
    ...
    return ('DSS REQUIRED', palette['dim']) if needs_dss else ('NO SURFACE SIGNALS', palette['dim'])
...
def _body_value_range(item):
    ...
    for row in item.get('genuses') or item.get('predicted_genuses') or []:
        values = [_safe_int(entry.get('value')) for entry in (row.get('species') or () if isinstance(row, dict) else ()) if entry.get('value')]
        if not values:
            info = bio_values.genus_info(_genus_name(row))
            values = [_safe_int(info.get(bound)) for bound in ('min_value', 'max_value') if info.get(bound)]
        if values:
...
def _joined_lines(values, max_chars=62):
    ...
    for raw in values:
        ...
        if not value:
            continue
        segments = textwrap.wrap(value, width=max_chars, break_long_words=False, break_on_hyphens=False) or [value]
...
def build_survey_model(system_name, scan_items, focused_body_id=None, focused_body_name=None, sampling=None, scanned=0, total=0, min_notable_value=50000, palette=None, total_known=True, body_signals=None, belt_clusters=None, show_all_bodies=False, dss_stats=None):
    """Build a renderer-neutral survey model for the overlay and tests."""
    bodies = _survey_bodies(scan_items, body_signals)
    # Scan rows are newest-first. Highlight the newest planet even when FSS
    # has already filled the system count and every scanned body is visible.
    latest_scan = next((row for row in scan_items or []
                        if not row.get('is_star') and row.get('planet_class')), None)

    def is_latest_scan(body):
        return bool(latest_scan and _body_matches(
            body, latest_scan.get('body_id'), latest_scan.get('name'),
        ))
    ...
    if focus_requested:
        ...
        return {'mode': 'body', 'system': system_name or '', 'body': focused, 'body_display': _body_display_name(focused.get('name'), system_name, focused.get('planet_class'), focused.get('terraformable')), 'rows': _body_detail_rows(focused), 'sampling': sampling, 'min_value': lo, 'max_value': hi, 'notable': focused_notable, 'notable_rows': [], 'scanned': _safe_int(scanned), 'total': _safe_int(total), 'total_known': bool(total_known), 'dss_stats': dss_stats or {}}
    rows = []
    ...
    if not rows and (not remaining_notable) and (not sampling):
        return None
    return {'mode': 'system', 'system': system_name or '', 'rows': rows, 'scope': 'all' if show_all_bodies else 'compact', 'notable_rows': remaining_notable, 'sampling': sampling, 'scanned': _safe_int(scanned), 'total': _safe_int(total), 'total_known': bool(total_known), 'notable_count': len(notable_rows), 'scan_in_progress': scan_in_progress, 'dss_stats': dss_stats or {}}
...
def _survey_render_key(model):
    ...
    if sampling:
        ...
        sampling_key = (sampling.get('species'), _safe_int(sampling.get('progress'), 1), sampling.get('colony_m'), minimum, sampling.get('clear'))
...
def _signal_node_states(signal_count, details=None, complete_count=0):
    ...
    complete = max(_safe_int(complete_count), sum((1 for row in details if row.get('kind') == 'complete')))
    sampled = sum((1 for row in details if row.get('kind') == 'sample'))
    detected = sum((1 for row in details if row.get('kind') == 'detected'))
    states = (['complete'] * complete + ['sample'] * sampled + ['detected'] * detected)[:signal_count]
...
class SurveyStatusHUD:
    ...
    def update(self, system_name, scanned, total, scan_items, body_signals, sampling=None, focused_body_id=None, focused_body_name=None, total_known=True, belt_clusters=None, dss_stats=None):
        self._last_update = (system_name, scanned, total, scan_items, body_signals, sampling, focused_body_id, focused_body_name, total_known, belt_clusters, dss_stats)
        if self._suppressed:
            return
        model = build_survey_model(system_name, scan_items, focused_body_id, focused_body_name, sampling, scanned, total, _safe_int(self.config.get('survey_min_body_value'), 50000), self._palette, total_known, body_signals=body_signals, belt_clusters=belt_clusters, show_all_bodies=bool(self.config.get('survey_status_show_all_bodies', False)), dss_stats=dss_stats)
        if not model:
            self._last_render_key = None
            self._html_render_model = None
            self.hide()
    ...
    def _compact_detail_text(detail):
        ...
        symbol = {'complete': '✓', 'sample': '●', 'detected': '○', 'predicted': '?', 'possible': '·'}.get(kind, '·')
        label = _truncate((detail or {}).get('display_name') or (detail or {}).get('name') or 'Organic', 31)
    ...
    def _sampling_matches_detail(sampling, detail):
        ...
        names = {str(detail.get(key) or '').casefold() for key in ('name', 'display_name')}
