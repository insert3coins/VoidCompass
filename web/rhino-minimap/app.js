(() => {
  ...
  function draw(model) {
    ...
    for(const mark of model.bookmarks || []) { const cx=px(mark.x),cy=py(mark.y),r=Math.max(4,side*.014),drill=mark.kind==="drill",colour=mark.depleted?(palette.red||"#ff6b70"):(drill?(palette.orange||"#ff8a3d"):(palette.green||"#54e39a")); if(cx<-r||cy<-r||cx>side+r||cy>side+r)continue;ctx.fillStyle=colour;ctx.strokeStyle=palette.bg||"#070b10";ctx.lineWidth=2;ctx.beginPath();if(drill){ctx.moveTo(cx,cy-r*1.35);ctx.lineTo(cx+r*1.35,cy);ctx.lineTo(cx,cy+r*1.35);ctx.lineTo(cx-r*1.35,cy);ctx.closePath();}else{ctx.arc(cx,cy,r,0,Math.PI*2);}ctx.fill();ctx.stroke();ctx.font=`bold ${Math.max(10,side*.038)}px Consolas`;ctx.textBaseline="middle";ctx.lineWidth=3;ctx.strokeText(mark.code||"",cx+r+3,cy);ctx.fillText(mark.code||"",cx+r+3,cy); }
  ...
  function render(snapshot = {}) {
    ...
    set("center-key", `SET CENTER · ${model.hotkeys?.center || "UNBOUND"}`); set("border-key", `SET BORDER · ${model.hotkeys?.border || "UNBOUND"}`); set("drill-key", `MARK DRILL · ${model.hotkeys?.drill || "UNBOUND"}`); set("reset-key", `RESET MAP · ${model.hotkeys?.reset || "UNBOUND"}`);
