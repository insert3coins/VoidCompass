...
function renderAchievementsWorkspace(data) {
  ...
  root.classList.remove("loading-panel");
  root.innerHTML = `${workspaceMetrics([{label: "Unlocked", value: `${numeric(data.unlocked)} / ${numeric(data.total)}`, detail: `${data.total ? numeric(number(data.unlocked) * 100 / number(data.total), 1) : "0.0"}% COMPLETE`}, {label: "Achievement points", value: numeric(data.points), detail: "PROFILE TOTAL"}, {label: "Categories", value: numeric((data.categories || []).length), detail: `${numeric((data.disabled || []).length)} DISABLED`}, {label: "Tracking", value: data.enabled ? "ACTIVE" : "PAUSED", detail: "JOURNAL ENGINE"}])}<div class="workspace-actions achievement-controls"><button data-ws-page="achievements" data-ws-op="set_enabled" data-enabled="${!data.enabled}">${data.enabled ? "PAUSE TRACKING" : "ENABLE TRACKING"}</button><input id="achievement-filter" placeholder="FILTER MILESTONES"></div><section id="achievement-grid" class="achievement-grid">${rows.join("") || `<p class="workspace-empty">Achievement catalogue unavailable.</p>`}</section>`;
}
...
function renderDashboard(state) {
  ...
  renderAtlas(model);
  text("rail-version", `v${model.app?.version || "5.3.9"} // WEBVIEW2`);
  text("boot-version", `v${model.app?.version || "5.3.9"} // SECURE LOOPBACK // WEBVIEW2`);
  text("about-version", `Version ${model.app?.version || "5.3.9"} // HTML Command Deck`);
