...
- **Rhino Coverage Minimap** opens automatically in the Rhino and paints the ground driven within the estimated 2 km scanner radius. Coverage maps, drop points, commander-set centers and borders persist by profile and body; the map includes north-up heading, coverage-driving rings, saved material-site markers, depletion state, scale, bearing and painted-area readouts. Because Elite does not publish drill deployment in the journal, an explicit button or configurable global hotkey marks each drill at the live Rhino position as a numbered orange diamond. Drill details remain editable in Planet Materials, and the map writes a shareable full-map PNG when the Rhino session ends.
