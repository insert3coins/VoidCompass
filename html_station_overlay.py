"""Purpose-built semantic HTML renderer for the Station Link overlay."""
...
def _json_safe(value):
    """Detach the journal-backed model from mutable dashboard containers."""
...
class HtmlStationOverlayBridge:
    ...
    def _model(self):
        model = getattr(self.overlay, "_last_model", None)
    ...
    def _snapshot(self):
        ...
        return {
            ...
            "station": _json_safe(self._model()),
    ...
    def set_enabled(self, enabled):
        ...
        try:
            self.surface = HtmlOverlaySurface(
                ...
                template="station", title=self.title,
            )
...
def attach_html_station_overlay(overlay, overlay_id, title, enabled_key, x_key, y_key):
    ...
    if overlay is None or getattr(overlay, "_html_station_bridge", None) is not None:
        return overlay
    bridge = HtmlStationOverlayBridge(
        overlay, overlay_id, title, enabled_key, x_key, y_key,
    )
    overlay._html_station_bridge = bridge
