...
def build_carrier_hud_model(carrier_data, now=None):
    ...
    badge, badge_tone = {'idle': ('READY', 'muted'), 'jumping': ('JUMPING', 'accent'), 'cooldown': ('COOLDOWN', 'yellow'), 'cooldown_cancel': ('CANCELLED', 'red')}.get(status, (status.upper(), 'muted'))
    ...
    remaining_fuel = sum((max(0, int(_number(row.get('fuel_used_t'), 0))) for row in route if not row.get('visited') and row.get('fuel_used_t') is not None))
    ...
    return {'carrier_type': 'SQUADRON CARRIER' if cd.get('carrier_type') == 'SquadronCarrier' else 'FLEET CARRIER', 'name': cd.get('name') or 'Fleet Carrier', 'callsign': cd.get('callsign') or '---', 'location': _fmt_location(cd.get('system'), cd.get('body')), 'status': status, 'badge': badge, 'badge_tone': badge_tone, 'movement_label': movement_label, 'movement_visible': movement_visible, 'movement_value': movement_value, 'movement_tone': movement_tone, 'movement_detail': movement_detail, 'route_name': str(cd.get('expedition_name') or 'EXPEDITION ROUTE'), 'route_total': len(route), 'route_done': done, 'route_complete': bool(route) and done == len(route), 'remaining_fuel': remaining_fuel, 'fuel': fuel, 'fuel_capacity': capacity, 'fuel_ratio': fuel_ratio, 'fuel_tone': fuel_tone, 'fuel_estimated': bool(cd.get('fuel_level_estimated')), 'range': current_range if current_range is not None else maximum_range, 'range_is_max': current_range is None and maximum_range is not None, 'cargo': cargo, 'free': free, 'orders': len(orders) if isinstance(orders, (list, tuple, dict)) else 0}
...
class CarrierHUD:
    ...
    def _schedule_tick(self):
        if self._tick_job:
            try:
                ...
                pass
        self._tick_job = self.win.call_later(1000, self._tick)
    ...
    def _build_rows(self, cd):
        ...
        rows = [(f"{_truncate(str(model['name']).upper(), 30)}  [{model['callsign']}]", palette['text']), (f"LOC: {_truncate(model['location'], 42)}", palette['text']), (f"{model['movement_label']}: {_truncate(model['movement_value'], 35)}", palette[model['movement_tone']])]
    ...
    def _schedule_config_save(self):
        if self._save_job:
            try:
                ...
                pass
        self._save_job = self.win.call_later(250, self._flush_config_save)
